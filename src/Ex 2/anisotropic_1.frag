...
subroutine(ill_model)
vec4 Heidrich_Seidel_ML_TX() // this name is the one which is detected by the SetupShaders() function in the main application, and the one used to swap subroutines
{
    // we repeat the UVs and we sample the texture
    vec2 repeated_Uv = mod(interp_UV*repeat, 1.0);
    vec4 surfaceColor = texture(tex, repeated_Uv);
    ...
    for(int i = 0; i < NR_LIGHTS; i++)
    {
        ...
        float TdotL = dot(T,L);
        float LdotNP = sqrt(1 - TdotL*TdotL);
        ...
        if(NdotL > 0.0)
        {
            ...
            // We add diffusive (= color sampled from texture) and specular components to the final color
            // N.B. ): in this implementation, the sum of the components can be different than 1
            color += NdotL*(Kd * LdotNP * surfaceColor + vec4(Ks * specular * specularColor, 1.0));
        }
    }
    return color;
}
...
subroutine(ill_model)
vec4 Ward_ML_TX() // this name is the one which is detected by the SetupShaders() function in the main application, and the one used to swap subroutines
{
        // we repeat the UVs and we sample the texture
    vec2 repeated_Uv = mod(interp_UV*repeat, 1.0);
    vec4 surfaceColor = texture(tex, repeated_Uv);
    ...
    for(int i = 0; i < NR_LIGHTS; i++)
    {
        ...
        if(NdotL > 0.0)
        {
            ...
            float wTdotH_sqd = TdotH/alphaX; // weighted TdotH squared
            wTdotH_sqd = wTdotH_sqd * wTdotH_sqd;

            float wBdotH_sqd = BdotH/alphaY; // weighted BdotH squared
            ...
            // the rendering equation is:
            //integral of: BRDF * Li * (cosine angle between N and L)
            // BRDF in our case is: the sum of Lambert and Ward
            // Li is considered as equal to 1: light is white, and we have not applied attenuation. With colored lights, and with attenuation, the code must be modified and the Li factor must be multiplied to finalColor
            color += (lambert + specular)*NdotL;
        }
